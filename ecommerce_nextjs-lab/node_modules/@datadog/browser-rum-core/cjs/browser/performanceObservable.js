...
const resourceUtils_1 = require("../domain/resource/resourceUtils");
const firstInputPolyfill_1 = require("./firstInputPolyfill");
...
function createPerformanceObservable(configuration, options) {
    return new browser_core_1.Observable((observable) => {
        ...
        const observer = new PerformanceObserver((0, browser_core_1.monitor)((entries) => {
            ...
            if (isObserverInitializing) {
                timeoutId = (0, browser_core_1.setTimeout)(() => handlePerformanceEntries(entries.getEntries()));
            }
            else {
        ...
        try {
            ...
            if (fallbackSupportedEntryTypes.includes(options.type)) {
                if (options.buffered) {
                    timeoutId = (0, browser_core_1.setTimeout)(() => handlePerformanceEntries(performance.getEntriesByType(options.type)));
                }
                try {
        ...
        if (!supportPerformanceTimingEvent(RumPerformanceEntryType.FIRST_INPUT) &&
            ...
            ;
            ({ stop: stopFirstInputTiming } = (0, firstInputPolyfill_1.retrieveFirstInputTiming)(configuration, (timing) => {
...
function manageResourceTimingBufferFull(configuration) {
    if (!resourceTimingBufferFullListener && supportPerformanceObject() && 'addEventListener' in performance) {
        ...
        resourceTimingBufferFullListener = (0, browser_core_1.addEventListener)(configuration, performance, 'resourcetimingbufferfull', () => {
